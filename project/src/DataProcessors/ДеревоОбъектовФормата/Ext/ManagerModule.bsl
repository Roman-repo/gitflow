...
// Процедура заполняет дерево значений представляющее структуру выбранной версии формата.
// Параметры:
...
Процедура ПолучитьИерархическоеПредставлениеФормата(СтруктураПараметров, АдресВременногоХранилища) Экспорт
	...
	|	ЕстьNULL(СвойстваФормата.Обязательное, ЛОЖЬ) КАК Обязательное,
	|	ЕстьNULL(СвойстваФормата.НижняяГраница, 0) КАК НижняяГраница,   
	|	ВЫБОР КОГДА ЕстьNULL(СвойстваФормата.ЭтоГруппа, ЛОЖЬ) ТОГДА
	|		ЛОЖЬ 		
	|	ИНАЧЕ ЕстьNULL(СвойстваФормата.СоставнойТип, ЛОЖЬ) КОНЕЦ	КАК ЭтоСоставнойТип,
	|	ЕстьNULL(СвойстваФормата.Наименование, """")	КАК ИмяСоставногоТипа,
	|	ВЫБОР КОГДА СвойстваФормата.Ссылка IS NULL ТОГДА ЛОЖЬ
	|	КОГДА СвойстваФормата.ОбъектХранительСвойств = """"
	|		И ( СвойстваФормата.Родитель = ЗНАЧЕНИЕ(Справочник.СвойстваФормата.ПустаяСсылка)
	|			ИЛИ СвойстваФормата.Родитель.ОбъектХранительСвойств = """"
	|			) ТОГДА
	|		ИСТИНА 
	|	КОГДА СвойстваФормата.Родитель.Наименование = ""КлючевыеСвойства""
	|		ИЛИ СвойстваФормата.Наименование = ""КлючевыеСвойства"" ТОГДА ИСТИНА
	|	ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ 									КАК  ДоступноИзменение,
	|	ЕстьNULL(СвойстваФормата.ЭтоГруппа, ЛОЖЬ) КАК ЭтоГруппа,
	|	ВЫРАЗИТЬ(СвойстваФормата.Тип КАК Строка(1000))	КАК ТипСвойстваСтрокой,
	|	ВЫБОР КОГДА СвойстваФормата.Ссылка IS NULL ТОГДА """"
	|	КОГДА СвойстваФормата.ЭтоГруппа ТОГДА
	|		СвойстваФормата.ТипОбщегоСвойства 	
	|	ИНАЧЕ
	|		СвойстваФормата.Родитель.ТипОбщегоСвойства 	
	|	КОНЕЦ										КАК ТипОбщегоРеквизита,
	|	ВЫБОР  
	|	КОГДА СвойстваФормата.ЭтоГруппа И СвойстваФормата.ТипОбщегоСвойства = ЗНАЧЕНИЕ(Перечисление.ТипыОбщихРеквизитов.КлючевыеСвойства)
	|		ТОГДА 3
	|	КОГДА СвойстваФормата.ЭтоГруппа И 
	|		(СвойстваФормата.ТипОбщегоСвойства = ЗНАЧЕНИЕ(Перечисление.ТипыОбщихРеквизитов.ГруппаОбщихСвойств)
	|			ИЛИ СвойстваФормата.ТипОбщегоСвойства = ЗНАЧЕНИЕ(Перечисление.ТипыОбщихРеквизитов.ОбщаяТабличнаяЧасть))
	|		ТОГДА 9
	|	КОГДА СвойстваФормата.ЭтоГруппа И СвойстваФормата.ТипОбщегоСвойства = ЗНАЧЕНИЕ(Перечисление.ТипыОбщихРеквизитов.ПустаяСсылка)
	|		ТОГДА 1
	|	КОГДА
...
Процедура ИнициализацияПеременных(СтруктураПараметров)
	...
	ДеревоФормата.Колонки.Добавить("НижняяГраница", Новый ОписаниеТипов("Число"));
	ДеревоФормата.Колонки.Добавить("ЭтоГруппа", ТипБулево);
	...
	Пока ВыборкаПоИерархии.Следующий() Цикл
		...
		Если ВыборкаПоИерархии.ТипЗаписи() = ТипЗаписиЗапроса.ИтогПоИерархии Тогда
			...
			ИначеЕсли ВыборкаПоИерархии.ТипОбщегоРеквизита = Перечисления.ТипыОбщихРеквизитов.КлючевыеСвойства Тогда
				ЭлементИерархии.КодКартинки = ?(ВыборкаПоИерархии.Группа3.ЭтоГруппа, 4, 3);
			Иначе
				ЭлементИерархии.КодКартинки = 1;
			КонецЕсли;
			УровеньИерархии = УровеньИерархии + 1;
			ЭлементИерархии.ЭтоГруппа = Истина;
			ОбработатьВыборкуПоИерархии(ВыборкаПоИерархии, УровеньИерархии, ЭлементИерархии, СтруктураПараметров);
		Иначе
			Если ЭлементИерархии.ТипОбщегоРеквизита = Перечисления.ТипыОбщихРеквизитов.КлючевыеСвойства Тогда
				ЭлементИерархии.КодКартинки = ?(ВыборкаПоИерархии.Группа3.ЭтоГруппа, 4, 3);
			ИначеЕсли НЕ ЗначениеЗаполнено(ЭлементИерархии.КодКартинки) Тогда
				ЭлементИерархии.КодКартинки = ?(ВыборкаПоИерархии.Группа3.ЭтоГруппа, 1, 5);
			КонецЕсли;
			ЭлементИерархии.ЭтоГруппа = ВыборкаПоИерархии.Группа3.ЭтоГруппа;
			...
			Пока Выборка.Следующий() Цикл
				...
				Если ЭлементКонечный.ТипОбщегоРеквизита = Перечисления.ТипыОбщихРеквизитов.КлючевыеСвойства Тогда
					ЭлементКонечный.КодКартинки = ?(Выборка.Группа3.ЭтоГруппа, 4, 3);
				ИначеЕсли ЭлементКонечный.КодКартинки = 0 Тогда
					ЭлементКонечный.КодКартинки = ?(ВыборкаПоИерархии.Группа3.ЭтоГруппа, 1, 5);
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
	КонецЦикла;
...
Функция ДобавитьВеткуДерева(ВышестоящийУровень, ТекЭлемент, СтруктураПараметров, СтрокаВыборки = Неопределено, ДополнятьТаблицуПоиска = Ложь)
	...
	ЭлементУровня.НижняяГраница = 0;     
	ЭлементУровня.ДоступноИзменение = Ложь;
	Если СтрокаВыборки <> Неопределено Тогда
		ЗаполнитьЗначенияСвойств(ЭлементУровня, СтрокаВыборки, "ТипОбщегоРеквизита, ТипСвойстваСтрокой, Обязательное, НижняяГраница, КодКартинки, ДоступноИзменение");
