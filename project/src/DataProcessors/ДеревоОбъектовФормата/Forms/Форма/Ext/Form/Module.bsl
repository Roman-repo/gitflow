...
&НаКлиенте
Процедура РедактироватьСтруктуруФормата(Команда)
	РедактироватьСтруктуруФормата = НЕ РедактироватьСтруктуруФормата;
	УстановитьВидимость();
КонецПроцедуры

&НаКлиенте
Процедура ДобавитьСтроку(Команда)
	Результат = ОпределитьТекущийКонтекст(Истина);
...
Процедура ДобавитьГруппуСвойств(Команда)
	...
	Если Результат.ИмяОбъекта <> "СвойстваФормата" Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Добавление группы возможно только для свойств.'"));
		Возврат;
	КонецЕсли;
	Если НЕ Результат.ДоступноИзменение Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Добавление группы в данном контексте невозможно.'"));
		Возврат;
	КонецЕсли;
...
Процедура УдалитьСтроку(Команда)
	...
	КонецЕсли;
	Если НЕ Результат.ДоступноИзменение Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Удаление невозможно.'"));
		Возврат;
	КонецЕсли;
...
Процедура ПереместитьВверх(Команда)
	...
	КонецЕсли;
	Если Результат.ИмяОбъекта <> "СвойстваФормата" Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Перемещение доступно только для свойств формата.'"));
		Возврат;
	КонецЕсли;
	Если НЕ Результат.ДоступноИзменение Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Изменение порядка для данного свойства недоступно.'"));
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьДеревоФормата()
	...
	Попытка
		...
		ПараметрыФоновогоЗадания.НаименованиеФоновогоЗадания = НСтр("ru = 'Заполнение дерева объектов формата'");
		ПараметрыФоновогоЗадания.ЗапуститьВФоне = Истина;
		Результат = ДлительныеОперации.ВыполнитьВФоне("Обработки.ДеревоОбъектовФормата.ПолучитьИерархическоеПредставлениеФормата",
		...
		КонецЕсли;
	Исключение
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'При формировании дерева объектов формата произошла ошибка'") + ": " + ОписаниеОшибки());
		Возврат;
	КонецПопытки
...
Процедура УстановитьВидимость()
	...
	КонецЕсли;
	// Редактирование структуры формата.
	Элементы.ФормаРедактирование.Видимость = РедактироватьСтруктуруФормата;
	Для Каждого КомандаРедактирования Из Элементы.ФормаРедактирование.ПодчиненныеЭлементы Цикл
		КомандаРедактирования.Видимость = РедактироватьСтруктуруФормата;
	КонецЦикла;
	Элементы.ФормаРедактироватьСтруктуруФормата.Пометка = РедактироватьСтруктуруФормата;
КонецПроцедуры
...
Процедура ОбновитьДанныеТекущейСтроки()
	...
	КонецЕсли;
	ДанныеСтрокиДерева = Элементы.ДеревоФормата.ТекущиеДанные;
	ВозможноПустое = НЕ ДанныеСтрокиДерева.Обязательное;
	НижняяГраница = ДанныеСтрокиДерева.НижняяГраница;
...
#Область РедактированиеДерева
&НаКлиенте
Функция ОпределитьТекущийКонтекст(ДобавлениеСтроки = Ложь)
	Если ДеревоФормата.ПолучитьЭлементы().Количество() = 0 Тогда
	...
	ЭлементРодитель2 = Неопределено;
	Если ВеткаРодитель <> Неопределено Тогда
		ЭлементРодитель = ВеткаРодитель.Элемент;
		ВеткаРодитель2 = ВеткаРодитель.ПолучитьРодителя();
		Если ВеткаРодитель2 <> Неопределено Тогда
			ЭлементРодитель2 = ВеткаРодитель2.Элемент;
		КонецЕсли;
	КонецЕсли;
	...
	ИначеЕсли ТипЗнч(ТекЭлемент) = Тип("СправочникСсылка.СвойстваФормата") Тогда
		...
		Если ТипЗнч(ЭлементРодитель) = Тип("СправочникСсылка.ОбъектыФормата") Тогда
			Результат.Вставить("ОбъектФормата", ЭлементРодитель);
			Если ТекДанные.ЭтоГруппа Тогда
				Результат.Вставить("Родитель", ТекЭлемент);
			КонецЕсли;
		ИначеЕсли ТипЗнч(ЭлементРодитель) = Тип("СправочникСсылка.СвойстваФормата")
			И ТипЗнч(ЭлементРодитель2) = Тип("СправочникСсылка.ОбъектыФормата") Тогда
			Результат.Вставить("ОбъектФормата", ЭлементРодитель2);
			Результат.Вставить("Родитель", ЭлементРодитель);
		КонецЕсли;
		Результат.Вставить("ГруппаСвойств", Ложь);
		Результат.Вставить("ПредыдущееСвойство", ТекЭлемент);
		ЕстьОпределенность = Результат.Свойство("ОбъектФормата");
	...
	ИначеЕсли ТипЗнч(ТекЭлемент) = Тип("СправочникСсылка.ОбъектыФормата") Тогда
		...
		Если ДобавлениеСтроки Тогда
			// Вопрос что добавить - объект, свойство объекта или значение.
			СписокДляВыбора = Новый СписокЗначений;
			СписокДляВыбора.Добавить("ОбъектыФормата", НСтр("ru='Объект формата'"));
			СписокДляВыбора.Добавить("СвойстваФормата", НСтр("ru='Свойство формата'"));
			ДопПараметры = Новый Структура("ТекущийЭлемент, Результат, ЭлементРодитель", ТекЭлемент, Результат, ЭлементРодитель);
			ДопПараметры.Вставить("ТипОбщегоРеквизита", ТекДанные.ТипОбщегоРеквизита);
			СписокДляВыбора.ПоказатьВыборЭлемента(Новый ОписаниеОповещения("ВыбранОбъектДляДобавления", ЭтотОбъект, ДопПараметры));
			Возврат Неопределено;
		Иначе
			ЕстьОпределенность = Истина;
...
&НаКлиенте
Процедура ВыбранОбъектДляДобавления(ВыбранныйЭлемент, ДополнительныеПараметры) Экспорт
	Если ВыбранныйЭлемент = Неопределено Тогда
		Возврат;
	КонецЕсли;
	Результат = ДополнительныеПараметры.Результат;
	ТекЭлемент = ДополнительныеПараметры.ТекущийЭлемент;
	ЭлементРодитель = ДополнительныеПараметры.ЭлементРодитель;
	ТипОбщегоРеквизита = ДополнительныеПараметры.ТипОбщегоРеквизита;
	Если ВыбранныйЭлемент.Значение = "ОбъектыФормата" Тогда
		Если ЗначениеЗаполнено(ТипОбщегоРеквизита) Тогда
			Результат.Вставить("ТипОбщегоРеквизита", ТипОбщегоРеквизита);
			Результат.Вставить("ШаблонНаименования", ?(СокрЛП(ТипОбщегоРеквизита) = НСтр("ru = 'Группа общих свойств'"),"ОбщиеСвойства",""));
		ИначеЕсли ЭлементРодитель = "Документы" Тогда
			Результат.Вставить("ШаблонНаименования", "Документ.");
		ИначеЕсли ЭлементРодитель = "Справочники" Тогда
			Результат.Вставить("ШаблонНаименования", "Справочник.");
		КонецЕсли;
	ИначеЕсли ВыбранныйЭлемент.Значение = "СвойстваФормата" Тогда
		Результат.Вставить("ОбъектФормата", ТекЭлемент);
		Результат.Вставить("ПредыдущееСвойство", Неопределено);
	Иначе
		Возврат;
	КонецЕсли;
	Результат.ИмяОбъекта = ВыбранныйЭлемент.Значение;
	ОткрытьФормуСКонтекстом(Результат);
КонецПроцедуры
...
Процедура ОткрытьФормуСКонтекстом(СтруктураКонтекст)
	...
	ИначеЕсли СтруктураКонтекст.ИмяОбъекта = "СвойстваФормата" Тогда
		Если СтруктураКонтекст.ДоступноИзменение = Ложь Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Добавление свойства невозможно.'"));
			Возврат;
		КонецЕсли;
