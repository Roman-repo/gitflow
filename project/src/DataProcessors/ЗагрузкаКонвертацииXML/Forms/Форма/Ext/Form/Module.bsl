...
Процедура ВыполнитьЗагрузку(Команда)
	Если НЕ ЗначениеЗаполнено(Объект.ИмяФайлаПравил) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Не выбран файл с правилами конвертации'"));
		Возврат;
	КонецЕсли;
	Если СпособЗагрузкиКонвертации > 0 И НЕ ЗначениеЗаполнено(Объект.Конвертация) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Не выбрана конвертация для загрузки правил'"));
		Возврат;
	КонецЕсли; 
	Если НЕ ЗначениеЗаполнено(Объект.Конфигурация) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Не выбрана конфигурация источник'"));
		Возврат;
	КонецЕсли;
	Если НЕ КонфигурацииСовпадают И НЕ ЗначениеЗаполнено(Объект.КонфигурацияКорреспондент) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Не выбрана конфигурация получатель'"));
		Возврат;
	КонецЕсли;
	Если НЕ ЗначениеЗаполнено(АдресФайлаПравил) Тогда
		// Файл выбран не через диалог выбора файла, либо запуск формы в режиме тестирования.
		// Проверка существования файла.
		ФайлПравил = НайтиФайлы(Объект.ИмяФайлаПравил,,Ложь); 
		Если ФайлПравил.Количество() = 0 Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Не найден файл с правилами конвертации'"));
			Возврат;
		КонецЕсли;
		// Получение файла.
		АдресФайлаПравил = ПоместитьВоВременноеХранилище(Новый ДвоичныеДанные(Объект.ИмяФайлаПравил), УникальныйИдентификатор);
	КонецЕсли;
...
Процедура ЗавершениеВыбораФайлаПравил(Знач РезультатПомещенияФайлов, Знач ДополнительныеПараметры) Экспорт
	...
	Если ПустаяСтрока(ТекстОшибки) И ПустаяСтрока(АдресФайлаПравил) Тогда
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстОшибки, , "Объект.ИмяФайлаПравил");
	Иначе
...
Процедура ПолучитьДанныеКонвертацииИзФайла()
	...
	Попытка
		...
		Если Результат.Статус = "Выполнено" Тогда
			...
			Иначе
				ОбщегоНазначения.СообщитьПользователю(НСтр("ru='Ошибка при получении данных из правил конвертации XML'"));
			КонецЕсли;
			Попытка
		...
		КонецЕсли;
	Исключение
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Ошибка при получении данных из правил конвертации XML'") + Символы.ПС + ОписаниеОшибки());
		Возврат;
	КонецПопытки 
...
Процедура ОкончаниеЗагрузкиКонвертации()
	...
	Если РезультатЗагрузки.ЗагрузкаШапки Тогда
		Если РезультатЗагрузки.Успешно Тогда
			ЗаполнитьДанныеФормыПоДаннымФайлаПравил(РезультатЗагрузки);
		Иначе
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Ошибка чтения правил конфигурации'"));
	...
	Иначе
		Если РезультатЗагрузки.Успешно Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Загрузка правил конвертации выполнена успешно'"));
		Иначе
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Загрузка правил конвертации не выполнена'"));
		КонецЕсли;
	КонецЕсли;
...
Процедура ВыполнитьЗагрузкуСервер()
	...
	Попытка
		...
		Если Результат.Статус = "Выполнено" Тогда
			РезультатЗагрузки = ПолучитьИзВременногоХранилища(АдресХранилищаРезультата);
			Если РезультатЗагрузки.Успешно Тогда
				ОбщегоНазначения.СообщитьПользователю(НСтр("ru='Загрузка правил конвертации выполнена успешно'"));
			Иначе
				ОбщегоНазначения.СообщитьПользователю(НСтр("ru='Загрузка правил конвертации не выполнена'"));
			КонецЕсли;
			Попытка
				УдалитьФайлы(ИмяФайлаПравил);
			Исключение
		...
		КонецЕсли;
	Исключение
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'При загрузке правил конвертации произошла ошибка'") + Символы.ПС + ОписаниеОшибки());
		Возврат;
	КонецПопытки
КонецПроцедуры
