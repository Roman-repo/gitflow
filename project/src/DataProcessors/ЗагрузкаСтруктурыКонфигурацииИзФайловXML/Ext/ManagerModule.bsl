...
Процедура ВыполнитьЗагрузкуМетаданных(ОбщиеПеременные, АдресВременногоХранилища) Экспорт
	...
	РассчитатьКоличествоОбъектовВКаталогахСДанными(ОбщиеПеременные);
	Если НЕ ОбщиеПеременные.ЗагрузитьНовую Тогда
		Запрос = Новый Запрос;
		Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	Ссылка
		|ИЗ Справочник.Объекты 
		|ГДЕ Владелец = &Конфигурация
		| И НЕ ЭтоГруппа И Родитель <> ЗНАЧЕНИЕ(Справочник.Объекты.ПустаяСсылка) И ПометкаУдаления = ЛОЖЬ";
		Если ОбщиеПеременные.ТолькоОбновитьПланыОбмена Тогда
			Запрос.Текст = Запрос.Текст + " И Тип = ЗНАЧЕНИЕ(Перечисление.ТипыОбъектов.ПланОбмена)";
		КонецЕсли;
		Запрос.УстановитьПараметр("Конфигурация", ОбщиеПеременные.Конфигурация);
		ОбщиеПеременные.Вставить("СуществующиеОбъекты", Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Ссылка"));
		Запрос = Новый Запрос;
		Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	Ссылка
		|ИЗ Справочник.Свойства
		|ГДЕ Владелец.Владелец = &Конфигурация  И ПометкаУдаления = ЛОЖЬ";
		Если ОбщиеПеременные.ТолькоОбновитьПланыОбмена Тогда
			Запрос.Текст = Запрос.Текст + " И Владелец.Тип = ЗНАЧЕНИЕ(Перечисление.ТипыОбъектов.ПланОбмена)";
		КонецЕсли;

		Запрос.УстановитьПараметр("Конфигурация", ОбщиеПеременные.Конфигурация);
		ОбщиеПеременные.Вставить("СуществующиеСвойства", Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Ссылка"));
		Запрос = Новый Запрос;
		Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	Ссылка
		...
		Запрос.УстановитьПараметр("Конфигурация", ОбщиеПеременные.Конфигурация);
		ОбщиеПеременные.Вставить("СуществующиеЗначения", Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Ссылка"));
	Иначе
	...
	Если ОбщиеПеременные.ЕстьРасширения Тогда
		...
		КонецЦикла;
	КонецЕсли;
	ДлительныеОперации.СообщитьПрогресс(99, НСтр("ru='Обработка загруженных данных.'"));
	...
	Если НЕ ОбщиеПеременные.ТолькоОбновитьПланыОбмена Тогда
		...
		ЧислоОбъектовВсегоЛок = ВсеПВР.Количество();
		ЧислоОбъектовОбработаноЛок = 0;
		СтрокаПрогрессаШаблон = НСтр("ru='Обработка планов видов расчета. Обработано %1% из %2%'");

		Пока Выборка.Следующий() Цикл
			ДобавитьТабличныеЧастиВПланВидовРасчета(Выборка.Ссылка, ОбщиеПеременные, ВсеПВР, Выборка.ЕстьПериодДействияБазовый);
			ЧислоОбъектовОбработаноЛок = ЧислоОбъектовОбработаноЛок + 1; 
			СтрокаПрогресса = СтрЗаменить(СтрокаПрогрессаШаблон, "%1%", ЧислоОбъектовОбработаноЛок);  
			СтрокаПрогресса = СтрЗаменить(СтрокаПрогресса, "%2%", ЧислоОбъектовВсегоЛок);
			ДлительныеОперации.СообщитьПрогресс(99, СтрокаПрогресса);
		КонецЦикла;
	КонецЕсли;
	...
	ЧислоОбъектовВсегоЛок = ОбщиеПеременные.ОтложенноеЗаполнениеТиповСвойств.Количество();
	ЧислоОбъектовОбработаноЛок = 0;
	СтрокаПрогрессаШаблон = НСтр("ru='Заполнение типов загруженных свойств. Обработано %1% из %2%'");
	...
	Для Каждого СвойстваИТипы Из ОбщиеПеременные.ОтложенноеЗаполнениеТиповСвойств Цикл
		...
		Для Каждого ТипСвойства Из ТипыСвойства Цикл
			Если СтрНайти(НРег(ТипСвойства), "definedtype.") > 0 И ОбщиеПеременные.Свойство("ОпределяемыеТипы") Тогда
				...
				КонецЕсли;
				Продолжить; 
			ИначеЕсли СтрНайти(НРег(ТипСвойства), ":characteristic.") > 0 Тогда
	...
	ЧислоОбъектовВсегоЛок = СвойстваТипаХарактеристика.Количество();
	ЧислоОбъектовОбработаноЛок = 0;
	СтрокаПрогрессаШаблон = НСтр("ru='Заполнение типов свойств Характеристика. Обработано %1% из %2%'");
	...
	Если НЕ ОбщиеПеременные.ТолькоОбновитьПланыОбмена И ОбщиеПеременные.ВариантЗагрузкиДвижений > 0 Тогда
		...
		ЧислоОбъектовВсегоЛок = ОбщиеПеременные.РегистраторыИРегистры.Количество();
		ЧислоОбъектовОбработаноЛок = 0;
		СтрокаПрогрессаШаблон = НСтр("ru='Заполнение движений по регистрам у документов и типов регистраторов у регистров. Обработано %1% из %2%'");
	...
	ОбщиеПеременные.ЧислоОбъектовВсего = ОбщиеПеременные.СуществующиеОбъекты.Количество() +  ОбщиеПеременные.СуществующиеСвойства.Количество() + ОбщиеПеременные.СуществующиеЗначения.Количество();
	ОбщиеПеременные.ЧислоОбъектовЗагружено = 0; 
	Если ОбщиеПеременные.ЧислоОбъектовВсего > 0 Тогда
		Процент = 0;
	Иначе
		Процент = 99;
	КонецЕсли;
	ДлительныеОперации.СообщитьПрогресс(Процент, НСтр("ru='Пометка на удаление неактуальных элементов.'"));
	ЧислоОбъектовВсегоЛок = ОбщиеПеременные.СуществующиеОбъекты.Количество() + ОбщиеПеременные.СуществующиеСвойства.Количество() + ОбщиеПеременные.СуществующиеЗначения.Количество();
	ЧислоОбъектовОбработаноЛок = 0;  
	СтрокаПрогрессаШаблон = НСтр("ru='Пометка на удаление неактуальных элементов. Помечено %1% из %2%'");
	// Пометка на удаление лишних объектов, свойств, значений.
	Для Каждого Ссылка Из ОбщиеПеременные.СуществующиеОбъекты Цикл
		КУдалению = Ссылка.ПолучитьОбъект();
		КУдалению.ПометкаУдаления = Истина;
		КУдалению.Записать();
		ОбщиеПеременные.ЧислоОбъектовЗагружено = ОбщиеПеременные.ЧислоОбъектовЗагружено + 1;
		Процент = ОбщиеПеременные.ЧислоОбъектовЗагружено * 100 / ОбщиеПеременные.ЧислоОбъектовВсего;
		ЧислоОбъектовОбработаноЛок = ЧислоОбъектовОбработаноЛок + 1;
		СтрокаПрогресса = СтрЗаменить(СтрокаПрогрессаШаблон, "%1%", ЧислоОбъектовОбработаноЛок);  
		СтрокаПрогресса = СтрЗаменить(СтрокаПрогресса, "%2%", ЧислоОбъектовВсегоЛок);
		ДлительныеОперации.СообщитьПрогресс(Процент, СтрокаПрогресса);
	КонецЦикла;
	Для Каждого Ссылка Из ОбщиеПеременные.СуществующиеСвойства Цикл
		КУдалению = Ссылка.ПолучитьОбъект();
		КУдалению.ПометкаУдаления = Истина;
		КУдалению.Записать();
		ОбщиеПеременные.ЧислоОбъектовЗагружено = ОбщиеПеременные.ЧислоОбъектовЗагружено + 1;
		...
		ДлительныеОперации.СообщитьПрогресс(Процент, СтрокаПрогресса);
	КонецЦикла;
	Для Каждого Ссылка Из ОбщиеПеременные.СуществующиеЗначения Цикл
		КУдалению = Ссылка.ПолучитьОбъект();
		КУдалению.ПометкаУдаления = Истина;
		КУдалению.Записать();
		ОбщиеПеременные.ЧислоОбъектовЗагружено = ОбщиеПеременные.ЧислоОбъектовЗагружено + 1;
...
Процедура РассчитатьКоличествоОбъектовВКаталогахСДанными(ОбщиеПеременные)
	ОбщиеПеременные.Вставить("ЧислоОбъектовВсего", 0);
	ОбщиеПеременные.Вставить("ЧислоОбъектовЗагружено", 0);  
	Если ОбщиеПеременные.ТолькоОбновитьПланыОбмена Тогда
		РассчитатьКоличествоОбъектов(ОбщиеПеременные, "ExchangePlans");
		Возврат;
	КонецЕсли;
	// Справочники.
	РассчитатьКоличествоОбъектов(ОбщиеПеременные, "Catalogs");
	// Документы.
	РассчитатьКоличествоОбъектов(ОбщиеПеременные, "Documents");
	// Перечисления.
	РассчитатьКоличествоОбъектов(ОбщиеПеременные, "Enums");
	// Задачи.
	РассчитатьКоличествоОбъектов(ОбщиеПеременные, "Tasks");
	// Бизнес процессы.
	РассчитатьКоличествоОбъектов(ОбщиеПеременные, "BusinessProcesses");
	// Планы счетов.
	РассчитатьКоличествоОбъектов(ОбщиеПеременные, "ChartsOfAccounts");
	// Планы видов расчетов.
	РассчитатьКоличествоОбъектов(ОбщиеПеременные, "ChartsOfCalculationTypes");
	// Планы видов характеристик.
	РассчитатьКоличествоОбъектов(ОбщиеПеременные, "ChartsOfCharacteristicTypes");
	// Регистры.
	РассчитатьКоличествоОбъектов(ОбщиеПеременные, "AccountingRegisters");
	РассчитатьКоличествоОбъектов(ОбщиеПеременные, "AccumulationRegisters");
	РассчитатьКоличествоОбъектов(ОбщиеПеременные, "CalculationRegisters");
	РассчитатьКоличествоОбъектов(ОбщиеПеременные, "InformationRegisters");
	// Планы обмена.
	РассчитатьКоличествоОбъектов(ОбщиеПеременные, "ExchangePlans");
	// Подписки на события.
	РассчитатьКоличествоОбъектов(ОбщиеПеременные, "EventSubscriptions");
	// Определяемые типы. 
	...
	Если ВложенныеФайлы.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	ОбщиеПеременные.ЧислоОбъектовВсего = ОбщиеПеременные.ЧислоОбъектовВсего + ВложенныеФайлы.Количество();
КонецПроцедуры

Процедура ОбработатьКаталогиСДанными(ОбщиеПеременные)
	Если ОбщиеПеременные.ТолькоОбновитьПланыОбмена Тогда
		ОбработатьГруппуОбъектов(ОбщиеПеременные, "ПланыОбмена", "ExchangePlans", "ПланОбменаСсылка", Перечисления.ТипыОбъектов.ПланОбмена);
		Возврат;
	КонецЕсли;
	// Справочники.
	ОбработатьГруппуОбъектов(ОбщиеПеременные, "Справочники", "Catalogs", "СправочникСсылка", Перечисления.ТипыОбъектов.Справочник);
	// Документы.
	ОбработатьГруппуОбъектов(ОбщиеПеременные, "Документы", "Documents", "ДокументСсылка", Перечисления.ТипыОбъектов.Документ);
	// Перечисления.
	ОбработатьГруппуОбъектов(ОбщиеПеременные, "Перечисления", "Enums", "ПеречислениеСсылка", Перечисления.ТипыОбъектов.Перечисление);
	// Задачи.
	ОбработатьГруппуОбъектов(ОбщиеПеременные, "Задачи", "Tasks", "ЗадачаСсылка", Перечисления.ТипыОбъектов.Задача);
	// Бизнес процессы.
	ОбработатьГруппуОбъектов(ОбщиеПеременные, "БизнесПроцессы", "BusinessProcesses", "БизнесПроцессСсылка", Перечисления.ТипыОбъектов.БизнесПроцесс);
	// Планы счетов.
	ОбработатьГруппуОбъектов(ОбщиеПеременные, "ПланыСчетов", "ChartsOfAccounts", "ПланСчетовСсылка", Перечисления.ТипыОбъектов.ПланСчетов);
	// Планы видов расчетов.
	ОбработатьГруппуОбъектов(ОбщиеПеременные, "ПланыВидовРасчета", "ChartsOfCalculationTypes", "ПланВидовРасчетаСсылка", Перечисления.ТипыОбъектов.ПланВидовРасчета);
	// Планы видов характеристик.
	ОбработатьГруппуОбъектов(ОбщиеПеременные, "ПланыВидовХарактеристик", "ChartsOfCharacteristicTypes", "ПланВидовХарактеристикСсылка", Перечисления.ТипыОбъектов.ПланВидовХарактеристик);
	// Регистры.
	ОбработатьГруппуОбъектов(ОбщиеПеременные, "РегистрыБухгалтерии", "AccountingRegisters", "РегистрБухгалтерииЗапись", Перечисления.ТипыОбъектов.РегистрБухгалтерии);
	ОбработатьГруппуОбъектов(ОбщиеПеременные, "РегистрыНакопления", "AccumulationRegisters", "РегистрНакопленияЗапись", Перечисления.ТипыОбъектов.РегистрНакопления);
	ОбработатьГруппуОбъектов(ОбщиеПеременные, "РегистрыРасчета", "CalculationRegisters", "РегистрРасчетаЗапись", Перечисления.ТипыОбъектов.РегистрРасчета);
	ОбработатьГруппуОбъектов(ОбщиеПеременные, "РегистрыСведений", "InformationRegisters", "РегистрСведенийЗапись", Перечисления.ТипыОбъектов.РегистрСведений);
	// Планы обмена.
	ОбработатьГруппуОбъектов(ОбщиеПеременные, "ПланыОбмена", "ExchangePlans", "ПланОбменаСсылка", Перечисления.ТипыОбъектов.ПланОбмена);
	// Подписки на события.
	ОбработатьГруппуОбъектов(ОбщиеПеременные, "ПодпискиНаСобытия", "EventSubscriptions", "ПодпискаНаСобытие", Перечисления.ТипыОбъектов.ПодпискаНаСобытие);
	// Общие реквизиты.
	ОбработатьГруппуОбъектов(ОбщиеПеременные, "ОбщиеРеквизиты", "CommonAttributes", "", Неопределено);

	// Константы.
	Процент = 99;
	Если ОбщиеПеременные.ЧислоОбъектовВсего > 0 Тогда
		Процент = Цел(ОбщиеПеременные.ЧислоОбъектовЗагружено * 100 / ОбщиеПеременные.ЧислоОбъектовВсего);
	КонецЕсли;
	ДлительныеОперации.СообщитьПрогресс(Процент,НСтр("ru='Загружаются константы '"));
...
Процедура ОбработатьГруппуОбъектов(ОбщиеПеременные, ИмяГруппы, ИмяКаталога, ПрефиксИменЭлементов, ТипОбъекта)
	...
	Если ВложенныеФайлы.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	ЧислоОбъектовВсегоЛок = ВложенныеФайлы.Количество();
	ЧислоОбъектовЗагруженоЛок = 0;
	Если ИмяГруппы = "ОбщиеРеквизиты" Тогда
		Для Каждого ВложенныйФайл Из ВложенныеФайлы Цикл
			...
			ОбщиеПеременные.ЧислоОбъектовЗагружено = ОбщиеПеременные.ЧислоОбъектовЗагружено + 1; 
			ЧислоОбъектовЗагруженоЛок = ЧислоОбъектовЗагруженоЛок + 1;
			Если ОбщиеПеременные.ЧислоОбъектовВсего > 0 Тогда
				Процент = Цел(ОбщиеПеременные.ЧислоОбъектовЗагружено * 100 / ОбщиеПеременные.ЧислоОбъектовВсего);
				СтрокаПрогресса = НСтр("ru='Загружаются объекты типа %1%. Загружено %2% из %3%'");
				СтрокаПрогресса = СтрЗаменить(СтрокаПрогресса, "%1%", ИмяГруппы);
				СтрокаПрогресса = СтрЗаменить(СтрокаПрогресса, "%2%", ЧислоОбъектовЗагруженоЛок); 
				СтрокаПрогресса = СтрЗаменить(СтрокаПрогресса, "%3%", ЧислоОбъектовВсегоЛок);
				ДлительныеОперации.СообщитьПрогресс(Процент,СтрокаПрогресса);
			КонецЕсли;
		КонецЦикла;
		Возврат;
...
Процедура ПрочитатьТипы(ЧтениеXML, ТекТипыСвойства, ПростойТипСтрокой, Квалификаторы)
	...
	Пока ЧтениеXML.Прочитать() Цикл
		...
		Если ТекИмяУзла = "type"
			...
			Прервать;
		ИначеЕсли ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
			Если ТекИмяУзла = "v8:type" Или ТекИмяУзла = "types" Или ТекИмяУзла = "v8:typeset" Тогда
...
Процедура ОбработатьОпределяемыеТипы(ОбщиеПеременные)
	...
	Для Каждого ВложенныйФайл Из ВложенныеФайлы Цикл
		...
		Если ОбщиеПеременные.ЧислоОбъектовВсего > 0 Тогда
			Процент = Цел(ОбщиеПеременные.ЧислоОбъектовЗагружено * 100 / ОбщиеПеременные.ЧислоОбъектовВсего);
		КонецЕсли;
		СтрокаПрогресса = НСтр("ru='Загружаются определяемые типы. Загружено %1% из %2%'");
		СтрокаПрогресса = СтрЗаменить(СтрокаПрогресса, "%1%", ЧислоОбъектовЗагруженоЛок);
		СтрокаПрогресса = СтрЗаменить(СтрокаПрогресса, "%2%", ЧислоОбъектовВсегоЛок);
		ДлительныеОперации.СообщитьПрогресс(Процент,СтрокаПрогресса);
	КонецЦикла;
...
Процедура ЗаписатьСообщениеОбОшибке(ОбщиеПеременные, ТекстСообщения)
	...
	ОбщиеПеременные.Ошибки = ТекстСообщения;

	Если ОбщиеПеременные.АдресВременногоХранилища <> Неопределено Тогда
		ВозвращаемоеЗначение = Новый Структура("Успешно, Ошибки", Ложь, НЕ ОбщиеПеременные.Отказ, ТекстСообщения);
...
Функция НайтиПомещенныеФайлыПоИмени(ОбщиеПеременные, ИмяФайла)
	...
	Для Каждого ТекФайл Из ОбщиеПеременные.ПомещенныеФайлы Цикл
		Если ТекФайл.ПолноеИмя = ИмяФайла 
			Или СтрНайти(ТекФайл.ПолноеИмя, ОбщиеПеременные.РазделительПути + ИмяФайла) > 0 Тогда
