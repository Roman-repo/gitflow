...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	КоллекцияЭлементовДерева = ДанныеДляУказания.ПолучитьЭлементы();
	...
	РассчитатьСтатусВсехОтметокДереваЗначений(ДанныеДляУказания);
...
&НаКлиенте
Процедура ДанныеДляУказанияОтметкаПриИзменении(Элемент)
	
	ИзменитьПометкуВыделеннойСтроки(ДанныеДляУказания, Элемент.Родитель.ТекущаяСтрока, Элемент.Родитель.ТекущиеДанные.Отметка);
...
&НаСервере
Функция ВыгрузитьДеревоЗначенийВТаблицуЗначений(ДеревоВыбранныхЗначений, ТаблицаВыбранныхЗначений, ПометкаВладельца = Неопределено)
...
&НаСервере
Процедура РассчитатьСтатусВсехОтметокДереваЗначений(ДеревоВыбранныхЗначений)
	
	Для Каждого СтрокаДерева Из ДеревоВыбранныхЗначений.ПолучитьЭлементы() Цикл
		
		Если СтрокаДерева.Отметка = 1 Тогда
			ИзменитьПометкуВыделеннойСтроки(ДанныеДляУказания, СтрокаДерева.ПолучитьИдентификатор() , 1);
		КонецЕсли;
		
		РассчитатьСтатусВсехОтметокДереваЗначений(СтрокаДерева);
...
Функция СформироватьТаблицуВыбранныхЗначений()
	...
	Возврат ПоместитьВоВременноеХранилище(
		ВыгрузитьДеревоЗначенийВТаблицуЗначений(ДанныеДляУказания, ТаблицаВыбранныхЗначений), 
