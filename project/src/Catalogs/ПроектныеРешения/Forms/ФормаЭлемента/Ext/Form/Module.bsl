...
Процедура ПриОткрытии(Отказ)
	...
	РежимВводаОбъекта = НСтр("ru='Выбор из списка'");
	РежимВводаСвойства = РежимВводаОбъекта;
	Если ЗначениеЗаполнено(Объект.ОбъектФормата) 
		...
		РежимВводаОбъекта = НСтр("ru='Ручной ввод'");
		РучнойВводОбъекта = Истина;
	КонецЕсли;
	Если ЗначениеЗаполнено(Объект.СвойствоФормата) 
		И НетСвойстваФорматаВТаблицеСвойств() Тогда
		РучнойВводСвойства = Истина;
		РежимВводаСвойства = НСтр("ru='Ручной ввод'");
	КонецЕсли;
...
Процедура ОбъектФорматаПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(Объект.СвойствоФормата) И НетСвойстваФорматаВТаблицеСвойств() Тогда
...
Процедура СвойствоФорматаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыФормыВыбора.Вставить("ТаблицаСвойствФормата", ТаблицаСвойствФормата);
	ОткрытьФорму("ОбщаяФорма.ВыборОбъектаИлиСвойства",ПараметрыФормыВыбора,Элемент,,,,
...
Процедура РежимВводаОбъектаПриИзменении(Элемент)
	РучнойВводОбъекта = (РежимВводаОбъекта = НСтр("ru='Ручной ввод'"));
	ОбновитьВнешнийВидФормы();
КонецПроцедуры

&НаКлиенте
Процедура РежимВводаСвойстваПриИзменении(Элемент)
	РучнойВводСвойства = (РежимВводаСвойства = НСтр("ru='Ручной ввод'"));
	ОбновитьВнешнийВидФормы();
КонецПроцедуры
...
Процедура ЗаполнитьСписокСвойств()
	ФильтрСвойств = Новый Структура("ОбъектФормата, ЭтоГруппа, ВерсияФормата",  Объект.ОбъектФормата, Ложь, ВерсияФормата);

	КонвертацияДанныхXDTOВызовСервера.ЗаполнитьСписокСвойствФормата(ТаблицаСвойствФормата, Неопределено, ФильтрСвойств);
КонецПроцедуры
...
Функция ПолучитьПредыдущуюВерсиюФормата()
	ТекВерсияФорматаЧислом = ВерсияФорматаЧислом(СокрЛП(Объект.Владелец.ВерсияФормата.Наименование));
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ 
	|	Наименование,
	|	Ссылка
	|ИЗ Справочник.ВерсииФормата
	|ГДЕ НЕ ПометкаУдаления И Ссылка <> &ЭтаВерсия И ЭтоГруппа = ЛОЖЬ И Родитель = ЗНАЧЕНИЕ(Справочник.ВерсииФормата.EnterpriseData)
...
Функция НетСвойстваФорматаВТаблицеСвойств()
	Возврат ТаблицаСвойствФормата.НайтиСтроки(Новый Структура("Имя", Объект.СвойствоФормата)).Количество() = 0;
КонецФункции
