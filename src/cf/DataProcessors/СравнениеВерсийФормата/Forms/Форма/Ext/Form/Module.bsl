...
Процедура ТекстТребованиеФорматОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	Если НавигационнаяСсылкаФорматированнойСтроки = "Еще" Тогда
		...
		ОткрытьФорму("Справочник.Требования.ФормаСписка", ПараметрыВыбора, ЭтотОбъект);
	КонецЕсли;
КонецПроцедуры

#КонецОбласти

#Область ОбработчикиСобытийЭлементовТаблицыФормыДеревоФормата
...
Процедура ОтчетОСравнении(Команда)
	Если НЕ ЗначениеЗаполнено(АдресХранилищаРезультата) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Сначала необходимо нажать кнопку Обновить данные.'"));
		Возврат;
	КонецЕсли;
	...
	ДиалогСохраненияФайла.Показать(Новый ОписаниеОповещения("ВыбранФайлДляСохраненияОтчета", ЭтотОбъект));
КонецПроцедуры
...
Процедура ОбновитьИнформациюОТребованиях()
	...
	КонецЕсли;
	ТекстТребованиеФормат = СтроковыеФункцииКлиент.ФорматированнаяСтрока(ТекстТребование);
КонецПроцедуры

&НаКлиенте
Процедура ВыбранФайлДляСохраненияОтчета(Знач ВыбранныеФайлы, Знач ДополнительныеПараметры) Экспорт
...
Процедура ОбработатьВерсииФормата()
	...
	СтруктураПараметров = Новый Структура("ВерсияФормата1, ВерсияФормата2, ВерсияФормата3, ПоказыватьТолькоИзменения",
						Объект.ВерсияФормата1, Объект.ВерсияФормата2, Объект.ВерсияФормата3, Объект.ПоказыватьТолькоИзменения);
	...
	Попытка
		...
		ПараметрыФоновогоЗадания.НаименованиеФоновогоЗадания = НСтр("ru = 'Сравнение версий формата'");
		ПараметрыФоновогоЗадания.ЗапуститьВФоне = Истина;
		Результат = ДлительныеОперации.ВыполнитьВФоне("Обработки.СравнениеВерсийФормата.ПолучитьРезультатСравненияВерсийФормата",
		...
		КонецЕсли;
	Исключение
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'При сравнении версий формата произошла ошибка'") + Символы.ПС + ОписаниеОшибки());
		Возврат;
	КонецПопытки
КонецПроцедуры
...
Функция ПроверитьЗаполнениеФормы()
	Если НЕ ЗначениеЗаполнено(Объект.ВерсияФормата1)
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Для сравнения необходимо указать две сравниваемые версии формата.'"));
		Возврат Ложь;
	ИначеЕсли Объект.ВерсияФормата1 = Объект.ВерсияФормата2 
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Для сравнения необходимо указать разные версии формата.'"));
		Возврат Ложь;
	КонецЕсли;
	Возврат Истина;
КонецФункции
...
Процедура ОтчетОСравненииНаСервере()
	...
	Если РезультатСравненияФормата.Строки.Количество() = 0 Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru='Сначала необходимо запустить сравнение версий.'"));
		Возврат;
	КонецЕсли;
