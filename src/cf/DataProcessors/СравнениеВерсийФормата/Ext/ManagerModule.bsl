...
Процедура СформироватьОтчетОСравнении(ДеревоФормата, АдресХранилищаРезультата, ВерсияФормата1, ВерсияФормата2) Экспорт
	...
	ТекстСодержание = ТекстСодержание + "<br><A href=""#Изменения_обязательных_реквизитов"">" + НСтр("ru='Изменения обязательных реквизитов'") + "</A>";
	ДобавитьТекстИзМакета(ТекстХТМЛ, "ИзмененияОбязательныхРеквизитов");
	ИмяРеквизитаСравнения = "Обязательное";
	МассивКолонок = Новый Массив;
	МассивКолонок.Добавить("ИмяОбъекта");
	МассивКолонок.Добавить("ИмяРеквизита");
	ТекстХТМЛБлок1 = "<H2>Были обязательные, стали необязательные</H2>";
	НарисоватьШапкуТаблицы(ТекстХТМЛБлок1, МассивКолонок);
	ТекстХТМЛБлок2 = "<H2>Были необязательные, обязательные</H2>";
	НарисоватьШапкуТаблицы(ТекстХТМЛБлок2, МассивКолонок);
	ТекстХТМЛБлок3 = "<H2>Новые обязательные</H2>";
	НарисоватьШапкуТаблицы(ТекстХТМЛБлок3, МассивКолонок);
	ДобавитьСводСравнениеПоШаблону(ТекстХТМЛБлок1, ТекстХТМЛБлок2, ТекстХТМЛБлок3, ДеревоФормата.Строки, 1, ИмяРеквизитаСравнения);
	// Закрыть таблицы.
	ТекстХТМЛБлок1 = ТекстХТМЛБлок1 + "</table><BR>" + Символы.ПС;
	ТекстХТМЛБлок2 = ТекстХТМЛБлок2 + "</table><BR>" + Символы.ПС;
	ТекстХТМЛБлок3 = ТекстХТМЛБлок3 + "</table><BR>" + Символы.ПС;
	ТекстХТМЛ = ТекстХТМЛ + ТекстХТМЛБлок1 + ТекстХТМЛБлок2 + ТекстХТМЛБлок3;
	
	ТекстСодержание = ТекстСодержание + "<br><A href=""#Изменения_возможно_пустое"">" + НСтр("ru='Изменения в передаче пустых значений'") + "</A>";
	ДобавитьТекстИзМакета(ТекстХТМЛ, "ИзмененияВозможноПустое");
	ИмяРеквизитаСравнения = "НижняяГраница";
	ТекстХТМЛБлок1 = "<H2>Нельзя было передавать пустыми, стало можно</H2>";
	НарисоватьШапкуТаблицы(ТекстХТМЛБлок1, МассивКолонок);
	ТекстХТМЛБлок2 = "<H2>Можно было передать пустыми, стало нельзя</H2>";
	НарисоватьШапкуТаблицы(ТекстХТМЛБлок2, МассивКолонок);
	ТекстХТМЛБлок3 = "<H2>Новые можно выгрузить пустыми</H2>";
	НарисоватьШапкуТаблицы(ТекстХТМЛБлок3, МассивКолонок);
	ДобавитьСводСравнениеПоШаблону(ТекстХТМЛБлок1, ТекстХТМЛБлок2, ТекстХТМЛБлок3, ДеревоФормата.Строки, 1, ИмяРеквизитаСравнения);
	// Закрыть таблицы.
	ТекстХТМЛБлок1 = ТекстХТМЛБлок1 + "</table><BR>" + Символы.ПС;
...
Процедура ИнициализацияПеременных(СтруктураПараметров)
	...
	Для НомерКолонки = 1 По 4 Цикл
		...
		РезультатСравненияВерсийФормата.Колонки.Добавить("НижняяГраница"+НомерКолонки, 			ТипБулево);
...
Процедура ОбновитьДеревоФормата(ТекВерсияФормата, СтруктураПараметров)
	СтруктураПараметров.Вставить("ТекВерсияФормата", ТекВерсияФормата);
	Уровень0 = СтруктураПараметров.РезультатСравненияВерсийФормата.Строки;
	// Общие.
	...
	|		КОГДА ТипОбщегоРеквизита = ЗНАЧЕНИЕ(Перечисление.ТипыОбщихРеквизитов.КлючевыеСвойства) ТОГДА
	|			&КлючевыеСвойстваПредставление
	...
	|	ЕстьNULL(СвойстваФормата.Порядок,0)		КАК Группа3Сортировка,
	|	ВЫБОР КОГДА ЗначенияФормата.Ссылка is null 
	|		ТОГДА ЛОЖЬ ИНАЧЕ ИСТИНА КОНЕЦ			КАК ЭтоЗначениеФасета,
	...
	|ПО ЗначенияФормата.Владелец = СвойстваФормата.Ссылка
	...
	|	ИЛИ ОбъектыФормата.ТипОбщегоРеквизита = ЗНАЧЕНИЕ(Перечисление.ТипыОбщихРеквизитов.КлючевыеСвойства))
	...
	Запрос.УстановитьПараметр("КлючевыеСвойстваПредставление", НСтр("ru = 'Ключевые свойства'"));
	...
	Запрос.МенеджерВременныхТаблиц = МенеджерВТ;
	Запрос.Текст = "ВЫБРАТЬ
	|	""Простые типы"" 			КАК Группа1,
	|	""Простые типы"" 			КАК Группа1Сортировка,
	...
	|	ЛОЖЬ 						КАК ЭтоСоставнойТип,
	|	ЛОЖЬ 						КАК ЭтоСвойствоФормата,
	|	Ссылка.Наименование			КАК Группа2Сортировка,
	...
	|УПОРЯДОЧИТЬ ПО Группа1 ВОЗР, Группа2Сортировка ВОЗР
	|ИТОГИ 
	|	МИНИМУМ(КодКартинки_Группа),
	|	МИНИМУМ(Группа2Сортировка)
	|ПО Группа1";
	...
	Запрос.МенеджерВременныхТаблиц = МенеджерВТ;
	Запрос.Текст = "ВЫБРАТЬ
	|	""Составные типы"" 			КАК Группа1,
	|	ТипыФормата.Ссылка.Наименование			КАК Группа2Сортировка,
	|	ТипыФормата.Ссылка 						КАК Группа2,
	|	0 							КАК НижняяГраница,
	|	ИСТИНА 						КАК ЭтоСоставнойТип,
	|	ЛОЖЬ 						КАК ЭтоСвойствоФормата,
	|	ТипыФорматаСостав.ТипФормата КАК Группа3,
	|	ТипыФорматаСостав.ТипФормата	КАК Группа3Сортировка,
	|	ЛОЖЬ 						КАК Обязательное,
	|	ТипыФорматаСостав.ТипФормата КАК ТипСвойстваСтрокой,
	|	NULL						КАК ТипОбщегоРеквизита,
	|	13							КАК КодКартинки_Группа,
	|	5 							КАК КодКартинки
	|ИЗ Справочник.ТипыФормата КАК ТипыФормата
	|ЛЕВОЕ СОЕДИНЕНИЕ 
	|	Справочник.ТипыФормата.Состав	КАК ТипыФорматаСостав
	|ПО ТипыФормата.Ссылка = ТипыФорматаСостав.Ссылка
	|ГДЕ ТипыФормата.Владелец = &ВерсияФормата 
	|	И ТипыФормата.СоставнойТип = ИСТИНА
	|	И НЕ ТипыФормата.ПометкаУдаления
	|УПОРЯДОЧИТЬ ПО Группа1 ВОЗР, Группа2Сортировка ВОЗР, Группа3Сортировка ВОЗР
	|ИТОГИ 
	|	МИНИМУМ(КодКартинки_Группа),
	|	МИНИМУМ(Группа2Сортировка)
	...
	|	ВЫБОР КОГДА ЕстьNULL(ОбъектыФормата.Наименование, """") ПОДОБНО ""Справочник%"" ТОГДА
	|		1
	|	КОГДА ЕстьNULL(ОбъектыФормата.Наименование, """") ПОДОБНО ""Документ%"" ТОГДА
	|		2
	...
	|	СвойстваФормата.Ссылка 						КАК Группа3,
	|	ЕстьNULL(СвойстваФормата.Порядок, 0)		КАК Группа3Порядок,
	|	ЕстьNULL(СвойстваФормата.Наименование, """")	КАК Группа3Сортировка,
	|	СвойстваФормата.Обязательное 				КАК Обязательное,
	|	СвойстваФормата.НижняяГраница				КАК НижняяГраница,
	|	ВЫБОР КОГДА СвойстваФормата.ЭтоГруппа ТОГДА ЛОЖЬ 
	|	ИНАЧЕ СвойстваФормата.СоставнойТип КОНЕЦ 	КАК ЭтоСоставнойТип,
	...
	|ПО ЗначенияФормата.Владелец = СвойстваФормата.Ссылка
	|ЛЕВОЕ СОЕДИНЕНИЕ 
	|	Справочник.ЗначенияФормата 					КАК ЗначенияФорматаОбъект
	|ПО ЗначенияФорматаОбъект.Владелец = ОбъектыФормата.Ссылка
	|ГДЕ ОбъектыФормата.ТипОбщегоРеквизита = ЗНАЧЕНИЕ(Перечисление.ТипыОбщихРеквизитов.ПустаяСсылка)
	|	И ОбъектыФормата.Владелец = &ВерсияФормата
	|	И ЗначенияФорматаОбъект.Ссылка IS NULL
	|	И НЕ ОбъектыФормата.ПометкаУдаления
	|УПОРЯДОЧИТЬ ПО Группа1Сортировка ВОЗР, Группа2Сортировка ВОЗР, Группа3Порядок ВОЗР, Группа3Сортировка ВОЗР
	|ИТОГИ Минимум(КодКартинки_Группа), Минимум(ЭтоСоставнойТип), Минимум(ЭтоСвойствоФормата), Минимум(ЭтоЗначениеФасета)
	|ПО Группа1, Группа2, Группа3 ИЕРАРХИЯ";
	Запрос.УстановитьПараметр("ВерсияФормата", ТекВерсияФормата);
	РезультатЗапроса = Запрос.Выполнить();
...
Процедура ОбработатьВыборкуПоИерархии(ТекущаяВыборка, УровеньИерархии, ТекущаяВеткаДерева, СтруктураПараметров)
	...
	Пока ВыборкаПоИерархии.Следующий() Цикл
		...
		Иначе
			...
			КонецЕсли;
			Если ВыборкаПоИерархии.ЭтоЗначениеФасета Тогда
				// Если тип свойства не простой тип то не разворачиваем по фасетам.
				ТипФорматаСсылка = Справочники.ТипыФормата.НайтиПоНаименованию(ВыборкаПоИерархии.ТипСвойстваСтрокой,,,СтруктураПараметров.ТекВерсияФормата);
				Если НЕ ЗначениеЗаполнено(ТипФорматаСсылка) Тогда
					ОбъектФорматаСсылка = Справочники.ОбъектыФормата.НайтиПоНаименованию(ВыборкаПоИерархии.ТипСвойстваСтрокой,,,СтруктураПараметров.ТекВерсияФормата);
					Если НЕ ЗначениеЗаполнено(ОбъектФорматаСсылка) Тогда
						ЗапросФасеты = Новый Запрос;
						ЗапросФасеты.Текст = "ВЫБРАТЬ Наименование, Ссылка
						|ИЗ Справочник.ЗначенияФормата
						|ГДЕ Владелец = &Свойство И НЕ ПометкаУдаления
						|УПОРЯДОЧИТЬ ПО Наименование";
						ЗапросФасеты.УстановитьПараметр("Свойство", ВыборкаПоИерархии.Группа3);
						ВыборкаФасеты = ЗапросФасеты.Выполнить().Выбрать();
						Пока ВыборкаФасеты.Следующий() Цикл
							СтруктураДанныхВыборки = Новый Структура("ТипСвойстваСтрокой, Обязательное, НижняяГраница, КодКартинки, ЭтоСоставнойТип, ЭтоСвойствоФормата");
							ЗаполнитьЗначенияСвойств(СтруктураДанныхВыборки, ВыборкаПоИерархии, "ТипСвойстваСтрокой, Обязательное, НижняяГраница, КодКартинки, ЭтоСоставнойТип, ЭтоСвойствоФормата");
							ЭлементФасет = ДобавитьВеткуДерева(ЭлементИерархии, ВыборкаФасеты.Ссылка, СтруктураПараметров, СтруктураДанныхВыборки);
						КонецЦикла;
						Продолжить;
					КонецЕсли;
				КонецЕсли;
...
Функция ДобавитьВеткуДерева(ВышестоящийУровень, ТекЭлемент, СтруктураПараметров, СтрокаВыборки = Неопределено)
	...
	Префикс = "";
	Если СтруктураПараметров.ПостфиксКолонок <> 1 Тогда
		Для Сч = 1 По СчетчикУровней Цикл
			Префикс = Префикс + Символы.Таб;
		КонецЦикла;
	КонецЕсли;
	ЭлементУровня["Элемент"+СтруктураПараметров.ПостфиксКолонок] = Префикс + СокрЛП(ТекЭлемент);
	Если СтрокаВыборки <> Неопределено Тогда
		СтруктураДанныхВыборки = Новый Структура("ТипСвойстваСтрокой, Обязательное, НижняяГраница, КодКартинки, ЭтоСоставнойТип, ЭтоСвойствоФормата");
		ЗаполнитьЗначенияСвойств(СтруктураДанныхВыборки, СтрокаВыборки, "ТипСвойстваСтрокой, Обязательное, НижняяГраница, КодКартинки, ЭтоСоставнойТип, ЭтоСвойствоФормата");
		...
		Если СтруктураДанныхВыборки.ТипСвойстваСтрокой <> Неопределено Тогда
			ЭлементУровня["ТипСвойстваСтрокой"+СтруктураПараметров.ПостфиксКолонок] = УпорядочитьТипы(СтруктураДанныхВыборки.ТипСвойстваСтрокой);
		КонецЕсли;
		...
		КонецЕсли;
		Если СтруктураДанныхВыборки.НижняяГраница <> Неопределено Тогда
			ЭлементУровня["НижняяГраница"+СтруктураПараметров.ПостфиксКолонок] = СтруктураДанныхВыборки.НижняяГраница;
		КонецЕсли;
...
Процедура СравнитьДеревоФормата(Строки, СтруктураПараметров)
	...
	Для Инд = 0 По Строки.Количество() - 1 Цикл
		...
		Если ЗначениеЗаполнено(ТекСтрока.Элемент1) Тогда
			Если ПроверятьВерсию2 И ЗначениеЗаполнено(ТекСтрока.Элемент2) Тогда
				Если ТекСтрока.Элемент1 = ТекСтрока.Элемент2
					...
					И ТекСтрока.НижняяГраница1 = ТекСтрока.НижняяГраница2
			...
			Если ПроверятьВерсию3 И ЗначениеЗаполнено(ТекСтрока.Элемент3) Тогда
				Если ТекСтрока.Элемент1 = ТекСтрока.Элемент3
					...
					И ТекСтрока.НижняяГраница1 = ТекСтрока.НижняяГраница3
				...
				Если ПроверятьВерсию2 И ЗначениеЗаполнено(ТекСтрока.Элемент2) Тогда
					...
					И ТекСтрока.НижняяГраница2 = ТекСтрока.НижняяГраница3
			...
			Если ПроверятьВерсию4 И ЗначениеЗаполнено(ТекСтрока.Элемент4) Тогда
				Если ТекСтрока.Элемент1 = ТекСтрока.Элемент4
					...
					И ТекСтрока.НижняяГраница1 = ТекСтрока.НижняяГраница4
				...
				Если ПроверятьВерсию2 И ЗначениеЗаполнено(ТекСтрока.Элемент2) Тогда
					...
					И ТекСтрока.НижняяГраница2 = ТекСтрока.НижняяГраница4
				...
				Если ПроверятьВерсию3 И ЗначениеЗаполнено(ТекСтрока.Элемент3) Тогда
					...
					И ТекСтрока.НижняяГраница3 = ТекСтрока.НижняяГраница4
		...
		Если ТекСтрока.РезультатСравнения1 = "-" ИЛИ ТекСтрока.РезультатСравнения2 = "-"
			...
			КонецЕсли;
		КонецЕсли;
		Если СтруктураПараметров.ПоказыватьТолькоИзменения Тогда
			// В ТипСвойстваСтрокой должны быть только отличающиеся значения.
			Если ТекСтрока.РезультатСравнения1 = "*" Тогда
				ЛокТипСвойстваСтрокой1 = ТекСтрока.ТипСвойстваСтрокой1;
				ЛокТипСвойстваСтрокой2 = ТекСтрока.ТипСвойстваСтрокой2;
				УбратьИдентичныеЗначенияТипов(ЛокТипСвойстваСтрокой1, ЛокТипСвойстваСтрокой2);
				Если ПроверятьВерсию3 И ТекСтрока.РезультатСравнения3 = "*" Тогда
					ЛокТипСвойстваСтрокой1_3 = ТекСтрока.ТипСвойстваСтрокой1;
					ЛокТипСвойстваСтрокой2_3 = ТекСтрока.ТипСвойстваСтрокой2;
					ЛокТипСвойстваСтрокой3_1 = ТекСтрока.ТипСвойстваСтрокой3;
					ЛокТипСвойстваСтрокой3_2 = ТекСтрока.ТипСвойстваСтрокой3;
					УбратьИдентичныеЗначенияТипов(ЛокТипСвойстваСтрокой1_3, ЛокТипСвойстваСтрокой3_1); 
					УбратьИдентичныеЗначенияТипов(ЛокТипСвойстваСтрокой2_3, ЛокТипСвойстваСтрокой3_2);
					ТекСтрока.ТипСвойстваСтрокой1 = СобратьТипыБезДублей(ЛокТипСвойстваСтрокой1, ЛокТипСвойстваСтрокой1_3);
					ТекСтрока.ТипСвойстваСтрокой2 = СобратьТипыБезДублей(ЛокТипСвойстваСтрокой2, ЛокТипСвойстваСтрокой2_3); 
					ТекСтрока.ТипСвойстваСтрокой2 = СобратьТипыБезДублей(ЛокТипСвойстваСтрокой3_1, ЛокТипСвойстваСтрокой3_2);
				Иначе
					ТекСтрока.ТипСвойстваСтрокой1 = ЛокТипСвойстваСтрокой1;
					ТекСтрока.ТипСвойстваСтрокой2 = ЛокТипСвойстваСтрокой2;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
...
Процедура ДобавитьПромежуточныеВерсии(МассивВерсийФормата, Версия1, Версия2)
	...
	|ГДЕ НЕ ПометкаУдаления И НЕ Ссылка В (&МассивВерсийФормата) И ЭтоГруппа = Ложь
	|	И Родитель = &Родитель
	|УПОРЯДОЧИТЬ ПО Наименование";
	Запрос.УстановитьПараметр("МассивВерсийФормата", МассивВерсийФормата);
	Запрос.УстановитьПараметр("Родитель", Версия1.Родитель);
...
Функция УпорядочитьТипы(Знач ПредставлениеТипаСтрокой)
	Если СтрНайти(ПредставлениеТипаСтрокой, ";") = 0 Тогда
		Возврат ПредставлениеТипаСтрокой;
	КонецЕсли;
	МасВсеТипы = Новый СписокЗначений();
	МасВсеТипы.ЗагрузитьЗначения(СтрРазделить(ПредставлениеТипаСтрокой, ";", Ложь));
	МасВсеТипы.СортироватьПоЗначению();
	Возврат СтрСоединить(МасВсеТипы.ВыгрузитьЗначения(),";");
КонецФункции

Процедура УбратьИдентичныеЗначенияТипов(ТипСвойстваСтрокой1, ТипСвойстваСтрокой2)
	Если СтрНайти(ТипСвойстваСтрокой1, ";") = 0 И СтрНайти(ТипСвойстваСтрокой2, ";") = 0 Тогда
		Возврат;
	КонецЕсли;
	МасВсеТипы1 = Новый СписокЗначений();
	МасВсеТипы1.ЗагрузитьЗначения(СтрРазделить(ТипСвойстваСтрокой1, ";", Ложь));
	МасВсеТипы2 = Новый СписокЗначений();
	МасВсеТипы2.ЗагрузитьЗначения(СтрРазделить(ТипСвойстваСтрокой2, ";", Ложь));
	
	МасРазныеТипы1 = Новый СписокЗначений();
	МасРазныеТипы2 = Новый СписокЗначений();
	
	Для Каждого ТекТип Из МасВсеТипы1 Цикл
		Если МасВсеТипы2.НайтиПоЗначению(ТекТип.Значение) = Неопределено Тогда
			МасРазныеТипы1.Добавить(ТекТип.Значение);
		КонецЕсли;
	КонецЦикла;
	Для Каждого ТекТип Из МасВсеТипы2 Цикл
		Если МасВсеТипы1.НайтиПоЗначению(ТекТип.Значение) = Неопределено Тогда
			МасРазныеТипы2.Добавить(ТекТип.Значение);
		КонецЕсли;
	КонецЦикла;
	ТипСвойстваСтрокой1 = СтрСоединить(МасРазныеТипы1.ВыгрузитьЗначения(),";");
	ТипСвойстваСтрокой2 = СтрСоединить(МасРазныеТипы2.ВыгрузитьЗначения(),";");
КонецПроцедуры

Функция СобратьТипыБезДублей(ТипСвойстваСтрокой1, ТипСвойстваСтрокой2)
	Если ТипСвойстваСтрокой1 = ТипСвойстваСтрокой2 Тогда
		Возврат ТипСвойстваСтрокой1;
	КонецЕсли;
	МасВсеТипы1 = Новый СписокЗначений();
	МасВсеТипы1.ЗагрузитьЗначения(СтрРазделить(ТипСвойстваСтрокой1, ";", Ложь));
	МасВсеТипы2 = Новый СписокЗначений();
	...
	Для Каждого ТекТип Из МасВсеТипы2 Цикл
		Если МасВсеТипы1.НайтиПоЗначению(ТекТип.Значение) = Неопределено Тогда
			МасВсеТипы1.Добавить(ТекТип.Значение);
		КонецЕсли;
	КонецЦикла;
	МасВсеТипы1.СортироватьПоЗначению();
	Возврат СтрСоединить(МасВсеТипы1.ВыгрузитьЗначения(),";");
КонецФункции
#КонецОбласти
...
Процедура ДобавитьСводСравнениеПоШаблону(ТекстХТМЛ1, ТекстХТМЛ2, ТекстХТМЛ3, СтрокиДерева, ТекущийУровень, ИмяРеквизитаСравнения)
	Для Каждого СтрокаДерева Из СтрокиДерева Цикл
		Если НЕ ЗначениеЗаполнено(СтрокаДерева.Элемент2) Тогда
			Продолжить;
		КонецЕсли;
		ЭтоОбъект = (ТекущийУровень = 3);
		Если Найти(СтрокаДерева.Элемент2, ".") > 0 Тогда
			ЭтоОбъект = Истина;
		КонецЕсли;
		ЭтоСвойство = НЕ ЭтоОбъект И (ТекущийУровень >= 3);
		Если НЕ ЭтоСвойство И НЕ ЭтоОбъект Тогда
			ДобавитьСводСравнениеПоШаблону(ТекстХТМЛ1, ТекстХТМЛ2, ТекстХТМЛ3, СтрокаДерева.Строки, ТекущийУровень+1, ИмяРеквизитаСравнения)
		ИначеЕсли ЭтоОбъект Тогда
			ДобавитьСводСравнениеПоШаблону(ТекстХТМЛ1, ТекстХТМЛ2, ТекстХТМЛ3, СтрокаДерева.Строки, ТекущийУровень+1, ИмяРеквизитаСравнения)
		ИначеЕсли ЭтоСвойство Тогда
			Значение1 = СтрокаДерева[ИмяРеквизитаСравнения + "1"];
			Значение2 = СтрокаДерева[ИмяРеквизитаСравнения + "2"];
			Если Значение1 = Значение2 Тогда
				Продолжить;
			КонецЕсли;
			Если ИмяРеквизитаСравнения = "Обязательное" Тогда
				ДобавлятьСтроку = Ложь;
				НомерТаблицы = 0;
				Если Значение1 = Истина И Значение2 = Ложь Тогда
					ДобавлятьСтроку = Истина;
					НомерТаблицы = 1;
				ИначеЕсли НЕ ЗначениеЗаполнено(СтрокаДерева.Элемент1) И Значение2 = Истина Тогда
					ДобавлятьСтроку = Истина;
					НомерТаблицы = 3;
				ИначеЕсли Значение1 = Ложь И Значение2 = Истина Тогда
					ДобавлятьСтроку = Истина;
					НомерТаблицы = 2;
				КонецЕсли;
			Иначе
				// Имя реквизита сравнения = нижняя граница
				ДобавлятьСтроку = Ложь;
				ТекТекстХТМЛ = "";
				Если Значение1 = 1 И Значение2 = 0 Тогда
					ДобавлятьСтроку = Истина;
					НомерТаблицы = 1;
				ИначеЕсли НЕ ЗначениеЗаполнено(СтрокаДерева.Элемент1) И Значение2 = 1 Тогда
					ДобавлятьСтроку = Истина;
					НомерТаблицы = 3;
				ИначеЕсли Значение1 = 0 И Значение2 = 1 Тогда
					ДобавлятьСтроку = Истина;
					НомерТаблицы = 2;
				КонецЕсли;
			КонецЕсли;
			...
			КонецЕсли;
			// Добавить строку таблицы.
			ИмяОбъекта = СтрокаДерева.Родитель.Элемент2;
			Если ТекущийУровень > 2 Тогда
				ИмяОбъекта = СтрокаДерева.Родитель.Родитель.Элемент2 + "." + ИмяОбъекта;
			КонецЕсли;
			Если ТекущийУровень > 3 Тогда
				ИмяОбъекта = СтрокаДерева.Родитель.Родитель.Родитель.Элемент2 + "." + ИмяОбъекта;
			КонецЕсли;
			Если ТекущийУровень > 4 Тогда
				ИмяОбъекта = СтрокаДерева.Родитель.Родитель.Родитель.Родитель.Элемент2 + "." + ИмяОбъекта;
			КонецЕсли;
			ТекстТаблицы = "<tr><td>" + ИмяОбъекта + "</td><td>" + СтрокаДерева.Элемент2 + "</td></tr>";
			Если НомерТаблицы = 1 Тогда
				ТекстХТМЛ1 = ТекстХТМЛ1 + Символы.ПС + ТекстТаблицы;
			ИначеЕсли НомерТаблицы = 2 Тогда
				ТекстХТМЛ2 = ТекстХТМЛ2 + Символы.ПС + ТекстТаблицы;
			ИначеЕсли НомерТаблицы = 3 Тогда
				ТекстХТМЛ3 = ТекстХТМЛ3 + Символы.ПС + ТекстТаблицы;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
...
Процедура ДобавитьИзменениеВТаблицу(ТекстХТМЛ, СтрокиДерева, ТекущийУровень, МассивТребований, ДобавлятьШапку = Ложь)
	Если ДобавлятьШапку Тогда
		...
		МассивКолонок.Добавить("Изменение");
		МассивКолонок.Добавить("Требование");
		НарисоватьШапкуТаблицы(ТекстХТМЛ, МассивКолонок);
	КонецЕсли;
	Для Каждого СтрокаДерева Из СтрокиДерева Цикл
		...
		ТекстИзменения = "";
		Если СтрокаДерева.РезультатСравнения1 = "*"
			И СтрокаДерева.РезультатСравнения2 = "*" Тогда
			Если СтрокаДерева.Обязательное1 <> СтрокаДерева.Обязательное2 Тогда
				ТекстИзменения = ?(СтрокаДерева.Обязательное1, "Стало необязательным", "Стало обязательным");
			КонецЕсли;
			Если СтрокаДерева.НижняяГраница1 <> СтрокаДерева.НижняяГраница2 Тогда
				Если ТекстИзменения <> "" Тогда
					ТекстИзменения = ТекстИзменения + "; ";
				КонецЕсли;
				ТекстИзменения = ТекстИзменения + "Нижняя граница " + СтрокаДерева.НижняяГраница1 + " -> " + СтрокаДерева.НижняяГраница2;
			КонецЕсли;
			Если СтрокаДерева.ТипСвойстваСтрокой1 <> СтрокаДерева.ТипСвойстваСтрокой2 Тогда
				Если ТекстИзменения <> "" Тогда
					ТекстИзменения = ТекстИзменения + "; ";
				КонецЕсли;
				Если ЗначениеЗаполнено(СтрокаДерева.ТипСвойстваСтрокой1) И ЗначениеЗаполнено(СтрокаДерева.ТипСвойстваСтрокой2) Тогда
					ТекстИзменения = ТекстИзменения + "Тип " + СтрокаДерева.ТипСвойстваСтрокой1 + " -> " + СтрокаДерева.ТипСвойстваСтрокой2;
				ИначеЕсли ЗначениеЗаполнено(СтрокаДерева.ТипСвойстваСтрокой1) И НЕ ЗначениеЗаполнено(СтрокаДерева.ТипСвойстваСтрокой2) Тогда
					ТекстИзменения = ТекстИзменения + "Из типа исключен " + СтрокаДерева.ТипСвойстваСтрокой1;
				Иначе
					ТекстИзменения = ТекстИзменения + "В тип добавлен " + СтрокаДерева.ТипСвойстваСтрокой2;
				КонецЕсли
			КонецЕсли;
		ИначеЕсли СтрокаДерева.РезультатСравнения1 = "+"
			Или СтрокаДерева.РезультатСравнения2 = "-" Тогда
			ТекстИзменения = "Удалено";
		ИначеЕсли СтрокаДерева.РезультатСравнения1 = "-"
			Или СтрокаДерева.РезультатСравнения2 = "+" Тогда
			ТекстИзменения = "Добавлено";
		КонецЕсли;
		ТребованиеТекст = ПредставлениеТребования(СтрокаДерева, МассивТребований);
		ТекстТаблицы = "<tr %ЦветФона%><td>" + СтрокаДокумента + "</td><td>" + ТекстИзменения + "</td><td>" + ТребованиеТекст + "</td></tr>";
		Если ЗначениеЗаполнено(ЦветФона) Тогда
			ТекстТаблицы = СтрЗаменить(ТекстТаблицы, "%ЦветФона%", "BGCOLOR=#"+ЦветФона);
		Иначе
			ТекстТаблицы = СтрЗаменить(ТекстТаблицы, "%ЦветФона%", "");
