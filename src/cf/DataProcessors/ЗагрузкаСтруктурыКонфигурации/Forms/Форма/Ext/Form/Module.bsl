...
Процедура ВыполнитьЗагрузку(Команда)
	Если НЕ ЗначениеЗаполнено(Объект.ИмяФайлаЗагрузки) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Не выбран файл со структурой конфигурации'"));
		Возврат;
	КонецЕсли;
...
Процедура УстановитьВидимость()
	...
	Элементы.ДобавлятьТолькоНовыеОбъектыСвойстваЗначения.Видимость = ЗагружатьВКонфигурацию;
КонецПроцедуры
...
Процедура ВыполнитьЗагрузкуСервер(СпособЗагрузки)
	...
	Попытка
		...
		ПараметрыФоновогоЗадания.НаименованиеФоновогоЗадания = НСтр("ru = 'Загрузка структуры метаданных'");
		ПараметрыФоновогоЗадания.ЗапуститьВФоне = Истина;
		Результат = ДлительныеОперации.ВыполнитьВФоне("Обработки.ЗагрузкаСтруктурыКонфигурации.ВыполнитьЗагрузкуМетаданных",
