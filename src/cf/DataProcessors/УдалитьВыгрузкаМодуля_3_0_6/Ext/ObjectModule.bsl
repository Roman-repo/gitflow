...
Функция ВыполнитьВыгрузкуМодулей() Экспорт
	...
	Если ВерсияФорматаМенеджера = "1" Тогда
		ТекстПроцедуры_ПКО = ЗаполнитьТекстПроцедурыПКО_Версия1(МакетТекстовМодулей, ТекстПроцедурыВызоваОбработчика);
	Иначе
...
Функция ЗаполнитьТекстПроцедурыПКО_Версия1(МакетТекстовМодулей, ТекстПроцедурыВызоваОбработчика)
	...
	Пока ВыборкаПоНаправлению.Следующий() Цикл
		...
		Пока ВыборкаДетальная.Следующий() Цикл
			...
			Если ТегЛокализации <> "" Тогда
			    Строка = "//++ " + ТегЛокализации + Символы.ПС;
			КонецЕсли;
			
			Если ТегСлужебный <> "" Тогда
				Строка = "//++" + ТегСлужебный + Символы.ПС;
			КонецЕсли;
			Строка = Строка + ?(ЕслиОткрыто, Символы.Таб, "") + "ДобавитьПКО_" + ВыборкаДетальная.Код + "(ПравилаКонвертации);";
			Если ТегСлужебный <> "" Тогда
				Строка = Строка + Символы.ПС +"//--" + ТегСлужебный;
			КонецЕсли;
			Если ТегЛокализации <> "" Тогда
				Строка = Строка + Символы.ПС + "//-- " + ТегЛокализации;
			КонецЕсли;
	...
	ТабПКС = РезультатЗапроса[3].Выгрузить();
	ТабПКС.Индексы.Добавить("ПравилоКонвертацииОбъекта");
	
	ТабПКС_ТЧ = РезультатЗапроса[4].Выгрузить();
	ТабПКС_ТЧ.Индексы.Добавить("ПравилоКонвертацииОбъекта");
	
	ТабПолейПоиска = РезультатЗапроса[5].Выгрузить();
	ТабПолейПоиска.Индексы.Добавить("ПравилоКонвертацииОбъекта");
...
Процедура ДобавитьОбработчикиВТекстМодуля(ПараметрыФормирования, Выборка, ТекстОбработчики, МакетТекстовМодулей, ТекстПроцедурыВызоваОбработчика, ТекстФункцииВызоваОбработчика = "")
	...
	Если СокрЛП(Выборка[ИмяОбработчика]) <> "" Тогда
		...
		Если ИмяОбработчика = "ВыборкаДанных" Тогда
			ТекстОбработчики = ТекстОбработчики + Символы.ПС + ?(ТекстОбработчики <> "" И НЕ ЗначениеЗаполнено(Комментарий), Символы.ПС,"")
